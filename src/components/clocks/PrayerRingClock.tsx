...
export function PrayerRingClock({
  ...
  return (
    <div className={cn(
      ...
      "rounded-full border-2 border-[var(--app-primary-container, hsl(var(--heroui-primary) / 0.15))] bg-content1 shadow-inner",
      ...
      {[
        ...
        <div key={pr.key} className="absolute inset-0 pointer-events-none" style={{ transform: `rotate(${pr.angle}deg)` }}>
          <div className={cn(
            "w-4 h-4 mx-auto mt-0.5 rounded-full shadow-[0_0_12px_var(--app-primary)] transition-all border-2 border-[var(--app-surface)] animate-pulse",
            "bg-primary",
            visualStyle === 'retro' && "bg-primary border border-white shadow-none rounded-none w-4.5 h-4.5 mt-0 animate-none",
            visualStyle === 'glass' && "bg-primary shadow-[0_0_8px_var(--app-primary)]",
            visualStyle === 'soft' && "bg-primary border-white/40"
          )}></div>
        </div>
      ))}
      ...
      {[...Array(12)].map((_, i) => (
        ...
        >
          <div className={cn(
            ...
            "bg-[var(--app-outline)]",
            visualStyle === 'retro' && "bg-[var(--app-foreground)] opacity-80 rounded-none w-1 h-3.5",
            visualStyle === 'glass' && "bg-[var(--app-foreground)] opacity-50",
            visualStyle === 'soft' && "bg-primary opacity-30"
          )}></div>
        </div>
      ))}
      ...
      <div className={cn(
        ...
        "bg-content4",
        visualStyle === 'retro' && "bg-content2 border border-[var(--app-foreground)] shadow-none rounded-none",
        visualStyle === 'glass' && "bg-white/5 border border-white/10 backdrop-blur-md",
        visualStyle === 'soft' && "bg-content1 shadow-[var(--soft-shadow-dark)]"
      )}>
         <span className={cn(
           "text-xl sm:text-2xl md:text-3xl font-sans font-black tracking-tight text-foreground tabular-nums select-none",
         ...
         {settings.timeFormat === "12h" && (
           <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-[var(--app-outline)]/60 -mt-1 select-none font-sans">
             {format(time, "a")}
           </span>
         )}
       </div>
      ...
      <div className="absolute inset-0 z-10 pointer-events-none">
        ...
        <div
          className={cn(
            ...
            visualStyle === 'retro' && "bg-[var(--app-foreground)] border border-white rounded-none w-4 sm:w-4.5 -ml-2 sm:-ml-[2.25px]",
            visualStyle === 'glass' && "bg-[var(--app-foreground)]",
            visualStyle === 'soft' && "bg-primary/90"
          )}
        ...
        <div
          className={cn(
            ...
            visualStyle === 'retro' && "bg-[var(--app-secondary)] border border-white rounded-none w-2.5 sm:w-3 -ml-[1.25px] sm:-ml-[1.5px]",
            visualStyle === 'glass' && "bg-[var(--app-foreground)]/80",
            visualStyle === 'soft' && "bg-[var(--app-secondary)]/90"
          )}
        ...
        >
          <div className={cn(
            ...
            "bg-[var(--app-danger)]",
            visualStyle === 'retro' && "bg-[var(--app-danger)] border border-white shadow-none rounded-none w-3.5 h-3.5 -left-1.5",
            visualStyle === 'glass' && "bg-[var(--app-danger)]",
            visualStyle === 'soft' && "bg-[var(--app-danger)]"
          )}></div>
        </div>
      </div>
      ...
      <div className={cn(
        ...
        "bg-content1 border-2 border-[var(--app-danger)]",
        visualStyle === 'retro' && "bg-white border-2 border-[var(--app-foreground)]",
        visualStyle === 'glass' && "bg-white border-[var(--app-foreground)]",
        visualStyle === 'soft' && "bg-white border-[var(--app-danger)]"
      )}></div>
    </div>
  );
}
