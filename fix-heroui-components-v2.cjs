 ...
 * 
 * Run with: node fix-heroui-components-v2.cjs
 */
...
// Regex generator for JSX tags that handles strings and {expressions} properly
const makeTagRegex = (tagName) => new RegExp(`<${tagName}([^>'"\\{]*(?:(?:'[^']*'|"[^"]*"|\\{[^\\}]*\\})[^>'"\\{]*)*)>([\\s\\S]*?)<\\/${tagName}>`, 'g');
const makeSelfClosingRegex = (tagName) => new RegExp(`<${tagName}([^>'"\\{]*(?:(?:'[^']*'|"[^"]*"|\\{[^\\}]*\\})[^>'"\\{]*)*)(?:>|\\/>)`, 'g');
const makeOpenTagRegex = (tagName) => new RegExp(`<${tagName}([^>'"\\{]*(?:(?:'[^']*'|"[^"]*"|\\{[^\\}]*\\})[^>'"\\{]*)*)>`, 'g');
...
function extractBoolAttribute(attrStr, attrName) {
  if (attrStr.match(new RegExp(`\\s${attrName}(?:\\s|=|$)`))) return true;
  return false;
}
...
function migrateComponents(filePath) {
  ...
  content = content.replace(makeTagRegex('md-outlined-text-field'), (m, attrs, inner) => {
    ...
    return `<input type=${type} ${placeholder} ${value} ${onInput} ${onChange} className={\`\${${className.replace(/^\{|\}$/g, '')}} px-4 py-3 rounded-2xl border-2 border-[var(--app-outline)]/20 bg-[var(--app-surface-container)] text-[var(--app-foreground)] placeholder:text-[var(--app-outline)] focus:border-[var(--app-primary)] focus:outline-none transition-colors\`} ref={inputRef} />`;
  });
